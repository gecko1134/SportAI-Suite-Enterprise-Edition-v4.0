...
class MembershipManager:
    ...
    def get_membership_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "new_this_month": 67,
            "churn_rate": 3.1,
            "avg_monthly_revenue": 187420,
            "membership_types": {
                "Venture North Club": 147,
                "All-Access Member": 743,
                "Family Plan": 312,
                "Basic Member": 587,
                "Community Advantage": 58
            },
            "retention_rate": 96.9,
            "player_lab_members": 23,
            "annual_revenue": 2249040
        }
...
def main():
    ...
    with tabs[0]:
        ...
        with col1:
            ...
            if PLOTLY_AVAILABLE:
                ...
                pivot_data = df_usage.pivot_table(index='Hour', columns='Facility', values='Usage %')
    ...
    with tabs[1]:
        ...
        with col1:
            ...
            if PLOTLY_AVAILABLE:
                ...
                # Fallback chart
                df_membership = pd.DataFrame(list(membership_types.items()), columns=['Tier', 'Members'])
                st.bar_chart(df_membership.set_index('Tier'))
            
            # Revenue by tier analysis - simplified for fallback
            st.markdown("### 💰 Revenue by Membership Tier")
            tier_revenue_data = {
                'Venture North Club': 1102500,
                'All-Access': 2229000,
                'Family Plan': 624000,
                'Basic': 440250,
                'Community Advantage': 0
            }
            
            revenue_df = pd.DataFrame(list(tier_revenue_data.items()), columns=['Tier', 'Revenue'])
            st.bar_chart(revenue_df.set_index('Tier'))
        ...
        if PLOTLY_AVAILABLE:
            ...
            st.line_chart(df_engagement.set_index('Date')[['Daily Visits', 'New Signups', 'Player Lab Usage']])
        ...
        with col1:
            ...
            for athlete in athlete_insights:
                ...
                with st.expander(f"{wellness_color} {athlete['athlete']} - {athlete['sport']}"):
                    ...
                    # Biometric chart - simplified for fallback
                    st.markdown("**📊 Weekly Biometrics:**")
                    ...
                    if PLOTLY_AVAILABLE:
                        fig_bio = px.line(bio_data, x='Day', y=['Heart Rate', 'Sleep Hours'],
                                         title=f"{athlete['athlete']} - Weekly Biometrics")
                        st.plotly_chart(fig_bio, use_container_width=True)
                    else:
                        st.line_chart(bio_data.set_index('Day'))
    ...
    with tabs[6]:
        ...
        with col1:
            ...
            if PLOTLY_AVAILABLE:
                ...
                # Fallback energy display
                st.markdown("**Energy Distribution:**")
                st.progress(energy_data['solar_generation'] / energy_data['total_consumption'])
                st.write(f"Solar: {energy_data['solar_generation']:.1f} kWh")
                st.progress(energy_data['grid_usage'] / energy_data['total_consumption'])
                st.write(f"Grid: {energy_data['grid_usage']:.1f} kWh")
    ...
    with tabs[7]:
        ...
        with col1:
            ...
            if PLOTLY_AVAILABLE:
                ...
                st.line_chart(df_revenue.set_index('Date')[['Revenue', 'Target', 'AI_Optimized']])
    ...
    with tabs[8]:
        ...
        with col1:
            ...
            if PLOTLY_AVAILABLE:
                ...
                # Fallback chart
                pivot_forecast = forecast_data.pivot_table(index='Day', columns='Facility', values='Predicted_Demand')
            ...
            if PLOTLY_AVAILABLE:
                ...
                st.bar_chart(df_engagement.set_index('Metric')[['This Month', 'Last Month']])
        ...
        if PLOTLY_AVAILABLE:
            ...
            st.markdown("**Community Impact Overview:**")
            for _, row in df_impact.iterrows():
                st.markdown(f"• **{row['Program']}**: {row['Participants']} participants, ${row['Investment ($)']/1000:.0f}K investment, {row['Impact Score']}/10 impact score")
    ...
    <div style='text-align: center; color: #666; padding: 20px;'>
        ...
        <p>System Status: 🟢 All systems operational | Last Updated: {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}</p>
        <p>Charts: {"🟢 Plotly Enhanced" if PLOTLY_AVAILABLE else "🟡 Streamlit Fallback"}</p>
    </div>
    """, unsafe_allow_html=True)
